...
#include "icons_font_awesome.h" // was #include "imgui-wrapper/icons_font_awesome.h"
#endif


#include <android/log.h>
#define  logRun2(...) ((void)__android_log_print(ANDROID_LOG_INFO, "RunGame", __VA_ARGS__))
...
//    const auto w0 =  ImGui::GetContentRegionAvail().y; //was ImGui::GetContentRegionAvailWidth();
//    const auto w1 = ImGui::CalcTextSize(text).x;
//    const auto p0 = ImGui::GetCursorScreenPos();
//    const ImVec2 vecL({p0.x + 0.5f*(w0 - w1), p0.y });
//    ImGui::SetCursorScreenPos(vecL);
//    if (disabled) {
...
std::thread initMainAndRunCore() {
    ...
    logRun2("we are in initMainAndRun ");
    return std::thread([&]() {
        while (g_isRunning) {
            updateCore();
...
void updateCore() {
    ...
    logRun2(" ksdjhfjkhs 22323 23 UPDATECORE ! ");
